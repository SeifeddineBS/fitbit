const Cardiac = require("../model/CardiacFrequency");
...
exports.create = async (req, res) => {
  ...
  const doesExist = await Cardiac.findOne({ id_patient: cardiac.id_patient });
...
exports.update = async (req, res) => {
  ...
  Cardiac.updateMany({"metadata":metadata},{"metadata":metadataNew}, { useFindAndModify: false })
  ...
  Cardiac
    ...
    .then((data) => {
      if (!data) {
        ...
        res.send({
          messagge: "Cardiac was deleted successfully", data
        });
      }
    })
    .catch((err) => {
      res.status(500).send({
        message: "Could not delete cardiac frequency with id=" + id,
      });
    });
};